...
public class Deserializer(public val value: Value) {
    ...
    public fun deserialize_unit(): Unit { if (!value.isNull()) throw Error("expected unit") }
    ...
    public fun deserialize_seq(): SeqAccess = when (val v = value.untag()) {
    ...
    public fun deserialize_map(): MapAccess = when (val v = value.untag()) {
    ...
    public fun deserialize_ignored_any(): Unit {}
}

public class SeqAccess internal constructor(private val iter: Iterator<Value>) {
    public fun next_element_seed(): Value? = if (iter.hasNext()) iter.next() else null
    public fun size_hint(): Pair<Int, Int?> = Pair(0, null)
}

public class MapAccess internal constructor(private val iter: Iterator<Map.Entry<Value, Value>>) {
...
public class EnumAccess(private val value: Value) {
    public fun variant_seed(): Pair<Value, VariantAccess> = when (value) {
...
public class VariantAccess(private val payload: Value) {
    public fun unit_variant(): Unit { if (!payload.isNull()) throw Error("expected unit variant") }
    public fun newtype_variant_seed(): Value = payload
    public fun tuple_variant(len: Int): SeqAccess = when (val v = payload.untag()) {
        is Value.Sequence -> SeqAccess(v.sequence.iterator())
        else -> throw Error("expected sequence")
    }
    public fun struct_variant(fields: Array<String>): MapAccess = when (val v = payload.untag()) {
        is Value.Mapping -> MapAccess(v.mapping.entries.iterator())
        else -> throw Error("expected mapping")
    }
}

public class UnitOnly(public val value: Value)
public class Tagged(public val tag: Tag, public val value: Value)

public fun <T> from_value(value: Value): T = io.github.kotlinmania.serdeyaml.fromValue(value)
public fun <T> fromValue(value: Value): T = io.github.kotlinmania.serdeyaml.fromValue(value)
