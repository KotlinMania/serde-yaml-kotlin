...
// port-lint: source serde_yaml/src/value/index.rs
...
public class IntIndex(
    ...
    override fun isKeyInto(v: Value): Boolean =
        when (val u = v.untag()) {
...
public fun index_into_mapping(index: Value, v: Value): Value? =
    when (val u = v.untagRef()) {
        is Value.Mapping -> u.mapping.get(index)
        else -> null
    }
...
public fun index_or_insert_mapping(index: Value, v: Value): Value =
    when (val u = v.untagMut()) {
