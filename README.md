...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`serde_yaml`](https://crates.io/crates/serde_yaml).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `serde_yaml` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `serde_yaml` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
