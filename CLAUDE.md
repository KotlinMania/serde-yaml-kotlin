# Claude Code Project Instructions — serde-yaml-kotlin

## Project Overview

This is **serde-yaml-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`serde_yaml`](https://crates.io/crates/serde_yaml).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/serde_yaml/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/serde_yaml>
